...
def parse_registry(
    csv_bytes: bytes, expected_registry: str
    ...
    for row in rdr:
        ...
        hexpart = "".join(
            c for c in assignment_raw if c in "0123456789ABCDEFabcdef"
        ).upper()
        if not hexpart:
            continue
...
def render_rust(
    phf24: List[Tuple[int, str]],
    phf28: List[Tuple[int, str]],
    phf36: List[Tuple[int, str]],
) -> str:
    lines: List[str] = []
    lines.append("// @generated by gen_oui_rust_phf.py — DO NOT EDIT.\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate Rust phf maps for MA-L(/24), MA-M(/28), MA-S(/36)"
    )
    ...
    mal = sorted(
        ((v, org) for (b, v), org in unique.items() if b == 24), key=lambda x: x[0]
    )
    mam = sorted(
        ((v, org) for (b, v), org in unique.items() if b == 28), key=lambda x: x[0]
    )
    mas = sorted(
        ((v, org) for (b, v), org in unique.items() if b == 36), key=lambda x: x[0]
    )
